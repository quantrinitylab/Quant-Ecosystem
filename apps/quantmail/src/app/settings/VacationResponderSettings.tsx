  ...
  return <div className="space-y-4 border-t border-[var(--quant-border)] pt-4">
    ...
    {status === 'loading' ? <p className="text-xs text-[var(--quant-muted-foreground)]">Loading live vacation responder…</p> : <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FormField label="Starts (optional)"><input type="datetime-local" className="w-full h-9 px-3 rounded-md border border-[var(--quant-border)] bg-[var(--quant-background)] text-sm text-[var(--quant-foreground)]" value={draft.startAt} disabled={status === 'saving'} onChange={(event) => setDraft((value) => ({ ...value, startAt: event.target.value }))} /></FormField>
        <FormField label="Ends (optional)"><input type="datetime-local" className="w-full h-9 px-3 rounded-md border border-[var(--quant-border)] bg-[var(--quant-background)] text-sm text-[var(--quant-foreground)]" value={draft.endAt} disabled={status === 'saving'} onChange={(event) => setDraft((value) => ({ ...value, endAt: event.target.value }))} /></FormField>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
