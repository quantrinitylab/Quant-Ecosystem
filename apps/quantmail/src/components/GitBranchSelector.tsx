 ...
 */
export function GitBranchSelector({ branches, currentBranch, onSwitchBranch, onCreateBranch }: GitBranchSelectorProps) {
            ...
            <div className="branch-list">
              {filtered.map((branch) => (
                <button
                  ...
                  onClick={() => { onSwitchBranch(branch.name); setIsOpen(false); }}
                >
                  <span className="branch-name">{branch.name}</span>
                  {branch.isDefault && <span className="branch-badge">default</span>}
                  {branch.isProtected && <span className="branch-badge branch-badge--protected">🔒</span>}
                  {branch.aheadBehind && (branch.aheadBehind.ahead > 0 || branch.aheadBehind.behind > 0) && (
                    <span className="branch-ahead-behind">
                      {branch.aheadBehind.ahead > 0 && <span className="branch-ahead">↑{branch.aheadBehind.ahead}</span>}
                      {branch.aheadBehind.behind > 0 && <span className="branch-behind">↓{branch.aheadBehind.behind}</span>}
                    </span>
                  )}
                  {branch.name === currentBranch && <span className="branch-check">✓</span>}
                </button>
              ))}
              {filtered.length === 0 && filter && (
                <button type="button" className="branch-create-inline" onClick={() => { setShowCreate(true); setNewBranchName(filter); }}>
                  Create branch "{filter}"
                </button>
              )}
            </div>
            {!showCreate && (
              <button type="button" className="branch-create-btn" onClick={() => setShowCreate(true)}>
                + New branch
              </button>
            )}
            {showCreate && (
