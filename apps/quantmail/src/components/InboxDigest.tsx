...
export function InboxDigest({ emails, isVisible }: InboxDigestProps) {
  const digest = useMemo(() => {
    ...
    for (const email of unread) {
      ...
      if (combined.includes('urgent') || combined.includes('asap') || combined.includes('immediate')) urgentCount++;
      if (combined.includes('meeting') || combined.includes('calendar') || combined.includes('invite')) meetingCount++;
      if (combined.includes('action') || combined.includes('please') || combined.includes('review')) actionCount++;
  ...
  return (
    ...
    >
      <div className="digest-header">
        <span className="digest-icon" aria-hidden="true">✦</span>
      ...
      <div className="digest-insights">
        {digest.urgentCount > 0 && (
          <span className="digest-chip digest-chip--urgent">
            🔴 {digest.urgentCount} urgent
          </span>
        )}
        {digest.actionCount > 0 && (
          <span className="digest-chip digest-chip--action">
            ⚡ {digest.actionCount} need action
          </span>
        )}
        {digest.meetingCount > 0 && (
          <span className="digest-chip digest-chip--meeting">
            📅 {digest.meetingCount} meeting{digest.meetingCount > 1 ? 's' : ''}
          </span>
        )}
        {digest.hasAttachments > 0 && (
          <span className="digest-chip digest-chip--attachment">
