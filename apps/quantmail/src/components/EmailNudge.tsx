...
const NUDGE_CONFIG: Record<EmailNudgeProps['type'], { icon: string; color: string; bg: string }> = {
  reply_needed: { icon: '↩️', color: '#60a5fa', bg: 'rgba(96,165,250,0.04)' },
  action_overdue: { icon: '⚡', color: '#f87171', bg: 'rgba(248,113,113,0.04)' },
  meeting_soon: { icon: '📅', color: '#a78bfa', bg: 'rgba(167,139,250,0.04)' },
  attachment_missing: { icon: '📎', color: '#fbbf24', bg: 'rgba(251,191,36,0.04)' },
};
...
export function EmailNudge({ type, message, onDismiss, onAction, actionLabel }: EmailNudgeProps) {
  ...
  return (
    ...
    >
      <span className="nudge-icon" style={{ color: config.color }}>{config.icon}</span>
      <span className="nudge-message">{message}</span>
      {onAction && actionLabel && (
        <button type="button" className="nudge-action" style={{ color: config.color }} onClick={onAction}>
          {actionLabel}
        </button>
      )}
      <button type="button" className="nudge-dismiss" onClick={onDismiss}>×</button>
    </motion.div>
  );
}
