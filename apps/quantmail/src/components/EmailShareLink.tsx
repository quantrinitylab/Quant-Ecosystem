...
export function EmailShareLink({ emailId, subject }: EmailShareLinkProps) {
  ...
  const generateLink = useCallback(() => {
    ...
    const fakeLink = `https://quantmail.quantrinity.in/shared/${emailId}?exp=${expiry}&sig=${Math.random().toString(36).slice(2, 10)}`;
    setLink(fakeLink);
  }, [emailId, expiry]);
    ...
    <div className="email-share-link">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <h4 className="share-link-title">Share "{subject.slice(0, 30)}{subject.length > 30 ? '…' : ''}"</h4>
            <div className="share-link-expiry">
              ...
              {(['1h', '24h', '7d', '30d'] as const).map((opt) => (
                <button
                  ...
                  onClick={() => { setExpiry(opt); setLink(null); }}
                >
                  {opt}
                </button>
              ))}
            ...
            {!link ? (
              ...
              <div className="share-link-result">
                ...
                <button type="button" className="share-link-copy" onClick={copyLink}>Copy</button>
              </div>
            )}
            <p className="share-link-note">Anyone with this link can view the email for the chosen duration.</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
