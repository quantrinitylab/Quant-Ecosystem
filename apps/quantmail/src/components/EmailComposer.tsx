...
export function EmailComposer({
  ...
  useEffect(() => {
    if (initialTo) {
      setToRecipients(parseInitialRecipients(initialTo));
    }
  }, [initialTo]);
  ...
  */
  ...
  const handleSend = async (scheduledAt?: string) => {
    ...
    // 1. Capture current form draft (to, subject, body, attachments)
    const draftSnapshot = {
      to: to.trim(),
    ...
    const toList =
      toRecipients.length > 0
        ? toRecipients.map((r) => ({ email: r.email }))
        : draftSnapshot.to
            .split(/[,;\s]+/)
            .filter(Boolean)
            .map((email) => ({ email }));
    ...
    const ccList =
      ccRecipients.length > 0
        ? ccRecipients.map((r) => ({ email: r.email }))
        : draftSnapshot.cc
          ? draftSnapshot.cc
              .split(/[,;\s]+/)
              .filter(Boolean)
    ...
    const bccList =
      bccRecipients.length > 0
        ? bccRecipients.map((r) => ({ email: r.email }))
        : draftSnapshot.bcc
          ? draftSnapshot.bcc
              .split(/[,;\s]+/)
              .filter(Boolean)
    ...
    // 2. Close composer modal/popover smoothly
    setIsDismissed(true);
    setShowSendOptionsDropdown(false);
    setIsQuantyDrawerOpen(false);
    ...
    queueSend({
      ...
      onUndo: async () => {
        // If user clicks "Undo" (or presses Z): reopen EmailComposer with the exact draft preserved (to, subject, body)
        setIsDismissed(false);
        setToRecipients(
          draftSnapshot.toRecipients.length > 0
            ? draftSnapshot.toRecipients
            : parseEmailString(draftSnapshot.to),
        );
        if (draftSnapshot.ccRecipients.length > 0) {
          setCcRecipients(draftSnapshot.ccRecipients);
          setShowCc(true);
        }
        if (draftSnapshot.bccRecipients.length > 0) {
          setBccRecipients(draftSnapshot.bccRecipients);
          setShowBcc(true);
        }
  ...
  const isVisible = (isOpen ?? true) && !isDismissed;
