...
export class RecurringService {
  ...
  parseRRule(rruleString: string): RecurrenceRule {
    ...
    if (values.has('EXDATE'))
      rule.exceptions = values
        .get('EXDATE')!
        .split(',')
        .map((value) => this.parseRRuleDate(value));
    return rule;
  }
  ...
  private fastForward(
    ...
    if (rule.frequency === 'monthly') {
      ...
      let candidate = this.shiftUtcMonths(
        seriesStart,
        jumps * rule.interval,
        seriesStart.getUTCDate(),
      );
      if (candidate > target && jumps > 0)
        candidate = this.shiftUtcMonths(
          seriesStart,
          (jumps - 1) * rule.interval,
          seriesStart.getUTCDate(),
        );
      return candidate;
    }
  ...
  private advanceDate(
    date: Date,
    rule: RecurrenceRule,
    targetDay: number = date.getUTCDate(),
  ): Date {
