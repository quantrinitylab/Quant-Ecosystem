...
export class EmailService {
   ...
   * `SuppressionService` over **this instance's own `prisma`** rather than falling back
   * to the module-level `suppressionService` singleton. That singleton is constructed
   * with no client, so it resolves `@quant/database`'s default `PrismaClient` — which
   * meant a caller who injected a mock/tenant-scoped client still had send-path
   * suppression checks hit the process-wide database and require `DATABASE_URL`.
   * Deriving from `this.prisma` keeps the injection seam honest end to end.
   */
