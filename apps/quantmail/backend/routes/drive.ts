...
async function usedBytes(prisma: any, userId: string): Promise<number> {
  const rows = await prisma.file.findMany({ where: { userId, isDeleted: false }, select: { size: true } });
  return rows.reduce((sum: number, row: { size: number }) => sum + (row.size || 0), 0);
}
...
function fileDto(file: FileRow, owner: Owner, decorations?: Decorations) {
  return {
    ...
    isStarred: file.isStarred, versions: decorations?.versions.get(file.id) ?? [],
  };
}
function folderDto(folder: FolderRow, owner: Owner, decorations?: Decorations) {
  return {
    ...
    isStarred: folder.isStarred, versions: [],
  };
}
async function loadDecorations(prisma: any, fileIds: string[], folderIds: string[]): Promise<Decorations> {
  ...
  for (const version of versions) {
    versionMap.set(version.fileId, [...(versionMap.get(version.fileId) ?? []), versionDto(version)]);
  }
...
function hashFromVersionKey(key: string): string | null {
  return key.match(/-([a-f0-9]{64})$/i)?.[1]?.toLowerCase() ?? null;
}
async function checkedPlaintext(row: any): Promise<Buffer> {
  const plaintext = decryptFromDrive(
    await getDriveObject(row.encryptedContent), row.encryptionIV, row.encryptionAuthTag, row.encryptionKey,
  );
...
const uploadSchema = z.object({
  name: z.string().min(1).max(255), mimeType: z.string().max(255).optional(),
  folderId: z.string().nullable().optional(), contentBase64: z.string().min(1),
});
...
export default async function driveRoutes(fastify: FastifyInstance) {
  fastify.get('/drive/quota', async (request, reply) => {
    ...
    return reply.send({ used: await usedBytes(getPrisma(fastify), userId), total: TOTAL_QUOTA });
  });

  fastify.get<{ Querystring: { folderId?: string } }>('/drive/files', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify);
    const folderId = request.query.folderId || null; const owner = await ownerInfo(prisma, userId);
    ...
    return reply.send({
      files: [...folders.map((folder: FolderRow) => folderDto(folder, owner, decorations)), ...files.map((file: FileRow) => fileDto(file, owner, decorations))],
      quota: { used: await usedBytes(prisma, userId), total: TOTAL_QUOTA },
    });
  });

  fastify.get<{ Querystring: { q?: string } }>('/drive/search', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const q = (request.query.q || '').trim();
  ...
  fastify.post('/drive/folders', async (request, reply) => {
    const parsed = z.object({ name: z.string().min(1).max(200), parentId: z.string().nullable().optional() }).safeParse(request.body);
    if (!parsed.success) throw parsed.error;
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const owner = await ownerInfo(prisma, userId);
    let path = `/${parsed.data.name}`;
    if (parsed.data.parentId) {
      const parent = await prisma.folder.findFirst({ where: { id: parsed.data.parentId, userId, isDeleted: false } });
  ...
  fastify.put<{ Params: { id: string }; Body: { name?: string } }>('/drive/files/:id', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const name = request.body?.name?.trim();
    ...
    if (item.type === 'file') await prisma.file.update({ where: { id: item.row.id }, data: { name } });
  ...
  fastify.put<{ Params: { id: string } }>('/drive/files/:id/star', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const item = await ownedItem(prisma, request.params.id, userId);
    const row = item.type === 'file'
  ...
  fastify.get<{ Params: { id: string } }>('/drive/files/:id/share', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const file = await fileAccess(prisma, request.params.id, userId);
    if (file.userId !== userId) throw createAppError('Only the owner can manage sharing', 403, 'FORBIDDEN');
  ...
  fastify.post<{ Params: { id: string } }>('/drive/files/:id/share', async (request, reply) => {
    ...
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const file = await fileAccess(prisma, request.params.id, userId);
    if (file.userId !== userId) throw createAppError('Only the owner can share this file', 403, 'FORBIDDEN');
    ...
    if (recipient.id === userId) throw createAppError('Cannot share a file with yourself', 400, 'INVALID_RECIPIENT');
    ...
    const share = existing
      ...
      : await prisma.share.create({ data: { ...data, fileId: file.id, folderId: null, ownerUserId: userId, sharedWithUserId: recipient.id } });
  ...
  fastify.delete<{ Params: { id: string; shareId: string } }>('/drive/files/:id/share/:shareId', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const file = await fileAccess(prisma, request.params.id, userId);
    if (file.userId !== userId) throw createAppError('Only the owner can revoke sharing', 403, 'FORBIDDEN');
    const share = await prisma.share.findFirst({ where: { id: request.params.shareId, fileId: file.id, ownerUserId: userId } });
  ...
  fastify.get<{ Params: { id: string } }>('/drive/files/:id/versions', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const file = await fileAccess(prisma, request.params.id, userId);
    const versions = await prisma.fileVersion.findMany({ where: { fileId: file.id }, orderBy: { versionNumber: 'desc' } });
  ...
  fastify.post<{ Params: { id: string } }>('/drive/files/:id/versions', { bodyLimit: DRIVE_MAX_BODY_BYTES }, async (request, reply) => {
    ...
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const file = await fileAccess(prisma, request.params.id, userId, true); requireStorage();
    ...
    if (bytes.length > DRIVE_MAX_FILE_BYTES) throw createAppError('File is larger than the upload limit', 413, 'FILE_TOO_LARGE');
    const used = await usedBytes(prisma, file.userId);
    if (used - file.size + bytes.length > TOTAL_QUOTA) throw createAppError('Storage quota exceeded', 507, 'QUOTA_EXCEEDED');
    const number = await nextVersion(prisma, file.id); const envelope = encryptForDrive(bytes);
    ...
    try {
      const version = await prisma.$transaction(async (tx: any) => {
        const created = await tx.fileVersion.create({ data: { fileId: file.id, versionNumber: number, encryptedContent: key, encryptionIV: envelope.iv, encryptionAuthTag: envelope.authTag, encryptionKey: envelope.wrappedKey, size: bytes.length } });
        await tx.file.update({ where: { id: file.id }, data: { encryptedContent: key, encryptionIV: envelope.iv, encryptionAuthTag: envelope.authTag, encryptionKey: envelope.wrappedKey, contentHash: envelope.contentHash, size: bytes.length, mimeType: parsed.data.mimeType ?? file.mimeType } });
        return created;
      });
  ...
  fastify.post<{ Params: { id: string; versionId: string } }>('/drive/files/:id/versions/:versionId/restore', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const file = await fileAccess(prisma, request.params.id, userId, true); requireStorage();
    const version = await prisma.fileVersion.findFirst({ where: { id: request.params.versionId, fileId: file.id } });
    if (!version) throw createAppError('Version not found', 404, 'VERSION_NOT_FOUND');
    const bytes = await checkedPlaintext(version); const hash = createHash('sha256').update(bytes).digest('hex');
  ...
  fastify.post<{ Body: { fileIds?: string[] } }>('/drive/files/trash', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const ids = [...new Set(request.body?.fileIds ?? [])]; const now = new Date();
    for (const id of ids) {
      ...
      if (folder) {
        ...
        await prisma.$transaction([
          prisma.folder.updateMany({ where: { id: { in: folderIds }, userId, isDeleted: false }, data: { isDeleted: true, deletedAt: now, trashRootId: id } }),
          prisma.file.updateMany({ where: { folderId: { in: folderIds }, userId, isDeleted: false }, data: { isDeleted: true, deletedAt: now, trashRootId: id } }),
        ]);
      }
    }
    for (const id of ids) await prisma.file.updateMany({ where: { id, userId, isDeleted: false }, data: { isDeleted: true, deletedAt: now, trashRootId: id } });
    return reply.send({ ok: true });
  });
  fastify.get('/drive/trash', async (request, reply) => {
    ...
    const [files, folders] = await Promise.all([
      ...
      prisma.folder.findMany({ where: { userId, isDeleted: true }, orderBy: { deletedAt: 'desc' } }),
    ]);
  ...
  fastify.post<{ Params: { id: string } }>('/drive/files/:id/restore', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const id = request.params.id;
    ...
    await prisma.$transaction([
      prisma.file.updateMany({ where: { userId, isDeleted: true, trashRootId: id }, data: { isDeleted: false, deletedAt: null, trashRootId: null } }),
      prisma.folder.updateMany({ where: { userId, isDeleted: true, trashRootId: id }, data: { isDeleted: false, deletedAt: null, trashRootId: null } }),
    ]);
    return reply.send({ ok: true });
  });
  fastify.delete<{ Params: { id: string } }>('/drive/files/:id/purge', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const id = request.params.id; requireStorage();
    const [files, folders] = await Promise.all([
      prisma.file.findMany({ where: { userId, isDeleted: true, trashRootId: id }, select: { id: true } }),
      prisma.folder.findMany({ where: { userId, isDeleted: true, trashRootId: id }, select: { id: true } }),
    ]);
    if (!files.length && !folders.length) throw createAppError('Trash item not found', 404, 'NOT_FOUND');
    await purgeRows(fastify, userId, files.map((file: any) => file.id), folders.map((folder: any) => folder.id));
  ...
  fastify.post('/drive/upload', { bodyLimit: DRIVE_MAX_BODY_BYTES }, async (request, reply) => {
    const parsed = uploadSchema.safeParse(request.body); if (!parsed.success) throw createAppError('Invalid upload payload', 400, 'VALIDATION_ERROR');
    const userId = requireUserId(request); const prisma = getPrisma(fastify); requireStorage();
    const bytes = Buffer.from(parsed.data.contentBase64, 'base64');
    if (!bytes.length) throw createAppError('Empty file', 400, 'VALIDATION_ERROR');
    if (bytes.length > DRIVE_MAX_FILE_BYTES) throw createAppError('File is larger than the upload limit', 413, 'FILE_TOO_LARGE');
    const used = await usedBytes(prisma, userId); if (used + bytes.length > TOTAL_QUOTA) throw createAppError('Storage quota exceeded', 507, 'QUOTA_EXCEEDED');
    let folderId = parsed.data.folderId ?? null;
    if (folderId && !(await prisma.folder.findFirst({ where: { id: folderId, userId, isDeleted: false } }))) folderId = null;
    const envelope = encryptForDrive(bytes);
    const file = await prisma.file.create({ data: { userId, name: safeFileName(parsed.data.name), mimeType: parsed.data.mimeType || 'application/octet-stream', size: bytes.length, folderId, encryptedContent: '', encryptionIV: envelope.iv, encryptionAuthTag: envelope.authTag, encryptionKey: envelope.wrappedKey, contentHash: envelope.contentHash } });
    const key = versionKey(userId, file.id, 1, envelope.contentHash);
    try {
      await putDriveObject(key, envelope.ciphertext);
      await prisma.$transaction([
        prisma.file.update({ where: { id: file.id }, data: { encryptedContent: key } }),
        prisma.fileVersion.create({ data: { fileId: file.id, versionNumber: 1, encryptedContent: key, encryptionIV: envelope.iv, encryptionAuthTag: envelope.authTag, encryptionKey: envelope.wrappedKey, size: bytes.length } }),
      ]);
    } catch (error) {
      await deleteDriveObject(key).catch(() => undefined); await prisma.file.delete({ where: { id: file.id } }).catch(() => undefined); throw error;
    }
    return reply.status(201).send({ file: fileDto({ ...file, folderId, encryptedContent: key }, await ownerInfo(prisma, userId)), quota: { used: used + bytes.length, total: TOTAL_QUOTA } });
  });

  fastify.get<{ Params: { id: string } }>('/drive/files/:id/download', async (request, reply) => {
    const userId = requireUserId(request); const file = await fileAccess(getPrisma(fastify), request.params.id, userId); requireStorage();
  ...
  fastify.post<{ Body: { fileIds?: string[]; targetFolderId?: string | null } }>('/drive/files/move', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const ids = request.body?.fileIds ?? []; const target = request.body?.targetFolderId ?? null;
    if (target && !(await prisma.folder.findFirst({ where: { id: target, userId, isDeleted: false } }))) throw createAppError('Target folder not found', 404, 'NOT_FOUND');
    await prisma.file.updateMany({ where: { id: { in: ids }, userId, isDeleted: false }, data: { folderId: target } });
    return reply.send({ ok: true });
  });

  fastify.post<{ Params: { id: string }; Body: { targetFolderId?: string | null } }>('/drive/files/:id/copy', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); requireStorage(); const source = await fileAccess(prisma, request.params.id, userId);
    const plaintext = await checkedPlaintext(source); const used = await usedBytes(prisma, userId);
    if (used + plaintext.length > TOTAL_QUOTA) throw createAppError('Storage quota exceeded', 507, 'QUOTA_EXCEEDED');
    const envelope = encryptForDrive(plaintext);
    const file = await prisma.file.create({ data: { userId, name: safeFileName(`${source.name} (copy)`), mimeType: source.mimeType, size: plaintext.length, folderId: request.body?.targetFolderId ?? source.folderId ?? null, encryptedContent: '', encryptionIV: envelope.iv, encryptionAuthTag: envelope.authTag, encryptionKey: envelope.wrappedKey, contentHash: envelope.contentHash } });
    const key = versionKey(userId, file.id, 1, envelope.contentHash);
    try {
      await putDriveObject(key, envelope.ciphertext);
      await prisma.$transaction([
        prisma.file.update({ where: { id: file.id }, data: { encryptedContent: key } }),
        prisma.fileVersion.create({ data: { fileId: file.id, versionNumber: 1, encryptedContent: key, encryptionIV: envelope.iv, encryptionAuthTag: envelope.authTag, encryptionKey: envelope.wrappedKey, size: plaintext.length } }),
      ]);
  ...
  fastify.delete<{ Params: { id: string } }>('/drive/files/:id', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const file = await prisma.file.findFirst({ where: { id: request.params.id, userId, isDeleted: true } });
    if (!file) throw createAppError('Move the file to trash before permanently deleting it', 409, 'NOT_TRASHED');
  ...
  fastify.delete<{ Params: { logicalId: string } }>('/drive/memory/:logicalId', async (request, reply) => {
    const userId = requireUserId(request); const prisma = getPrisma(fastify); const logicalId = request.params.logicalId;
    const owned = await prisma.memoryRecord.findFirst({ where: { logicalId, ownerType: 'user', ownerId: userId, deletedAt: null }, select: { id: true } });
    if (!owned) throw createAppError('Not found', 404, 'NOT_FOUND');
    const result = await prisma.memoryRecord.updateMany({ where: { logicalId, ownerType: 'user', ownerId: userId, archivedAt: null, deletedAt: null }, data: { archivedAt: new Date() } });
