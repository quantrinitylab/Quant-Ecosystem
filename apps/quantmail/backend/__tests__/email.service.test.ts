...
function createMockPrisma() {
  return {
    ...
    // The send path runs recipients through the Gate 4 suppression list. EmailService
    // derives that gate from this injected client, and SuppressionService deliberately
    // fails loudly (503 DATABASE_UNAVAILABLE) when the delegate is absent — so the mock
    // has to model it. Default: nothing suppressed.
    emailSuppression: {
