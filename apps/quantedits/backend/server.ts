/**
 * Production entrypoint for the QuantCooks (quantedits) Fastify backend.
 *
 ...
 * handlers all proxy to `QUANTEDITS_BACKEND_URL`, with nothing listening on the
...
async function main(): Promise<void> {
  if (process.env['NODE_ENV'] === 'test') {
    throw new Error(
      'FATAL: QuantCooks server cannot boot with NODE_ENV=test in standalone process',
    );
  }
  ...
  try {
    ...
    app.log.info({ port: config.port, host: config.host }, 'QuantCooks backend listening');
  } catch (err) {
    app.log.error({ err }, 'Failed to start QuantCooks backend');
    process.exit(1);
  }
}
