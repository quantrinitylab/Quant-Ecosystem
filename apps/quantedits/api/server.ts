// ============================================================================
// QuantEdits API Server
// Professional video/photo editor with AI tools and real-time collaboration
// ============================================================================
...
const defaultConfig: ServerConfig = {
  port: 3007,
  host: '0.0.0.0',
  corsOrigins: ['https://edits.quant.app', 'http://localhost:3000', 'http://localhost:3007'],
...
export class QuantEditsServer {
  ...
  private setupRoutes(): void {
    this.router.register(projectRoutes);
    this.router.register(templateRoutes);
    this.router.register(effectRoutes);
    this.router.register(exportRoutes);
    this.router.register(assetRoutes);
    this.router.register(aiRoutes);
    this.router.register(collaborationRoutes);
  }
  ...
  async handleRequest(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (route.requiresAuth) {
        ...
        req.user = { id: decoded.sub, email: decoded.email || '', username: decoded.username || '', role: decoded.role || 'user' };
      }
  ...
  getRoutes() {
    const allRoutes = [...projectRoutes, ...templateRoutes, ...effectRoutes, ...exportRoutes, ...assetRoutes, ...aiRoutes, ...collaborationRoutes];
    return allRoutes.map(r => ({ method: r.method, path: r.path, requiresAuth: r.requiresAuth ?? true }));
  }

  getHealthStatus() {
    return { status: 'healthy', service: 'quantedits', uptime: process.uptime(), version: '1.0.0', routes: this.getRoutes().length, features: ['video-editor', 'photo-editor', 'templates', 'effects', 'ai-editing', 'collaboration', 'export'] };
  }
  ...
  start(): void {
    ...
    server.listen(this.config.port, this.config.host, () => {
      console.log(`[QuantEdits] Server running at http://${this.config.host}:${this.config.port}`);
      console.log(`[QuantEdits] Routes registered: ${this.getRoutes().length}`);
    });
  }
}

export default QuantEditsServer;
