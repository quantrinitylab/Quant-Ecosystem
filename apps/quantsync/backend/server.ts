/**
 * Production entrypoint for the QuantWave (quantsync) Fastify backend.
 *
 ...
 * handlers all proxy to `QUANTSYNC_BACKEND_URL`, with nothing listening on the
...
async function main(): Promise<void> {
  if (process.env['NODE_ENV'] === 'test') {
    throw new Error('FATAL: QuantWave server cannot boot with NODE_ENV=test in standalone process');
  }
  ...
  try {
    ...
    app.log.info({ port: config.port, host: config.host }, 'QuantWave backend listening');
  } catch (err) {
    app.log.error({ err }, 'Failed to start QuantWave backend');
    process.exit(1);
  }
}
