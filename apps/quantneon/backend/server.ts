/**
 * Production entrypoint for the QuantGram (quantneon) Fastify backend.
 *
 ...
 * handlers all proxy to `QUANTNEON_BACKEND_URL`, with nothing listening on the
...
async function main(): Promise<void> {
  if (process.env['NODE_ENV'] === 'test') {
    throw new Error('FATAL: QuantGram server cannot boot with NODE_ENV=test in standalone process');
  }
  ...
  try {
    ...
    app.log.info({ port: config.port, host: config.host }, 'QuantGram backend listening');
  } catch (err) {
    app.log.error({ err }, 'Failed to start QuantGram backend');
    process.exit(1);
  }
}
