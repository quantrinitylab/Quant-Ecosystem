// ============================================================================
// QuantMax API Server
// Short video social, random video chat, and dating platform
// ============================================================================
...
const defaultConfig: ServerConfig = {
  port: 3008,
  host: '0.0.0.0',
  corsOrigins: ['https://max.quant.app', 'http://localhost:3000', 'http://localhost:3008'],
...
export class QuantMaxServer {
  ...
  private setupRoutes(): void {
    ...
    this.router.register(createRoutes);
    this.router.register(videochatRoutes);
    this.router.register(matchingRoutes);
    this.router.register(messageRoutes);
    this.router.register(profileRoutes);
    this.router.register(discoverRoutes);
    this.router.register(safetyRoutes);
  ...
  async handleRequest(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (route.requiresAuth) {
        ...
        req.user = { id: decoded.sub, email: decoded.email || '', username: decoded.username || '', role: decoded.role || 'user', verified: decoded.verified, ageVerified: decoded.ageVerified };
      }
  ...
  getRoutes() {
    const allRoutes = [...feedRoutes, ...createRoutes, ...videochatRoutes, ...matchingRoutes, ...messageRoutes, ...profileRoutes, ...discoverRoutes, ...safetyRoutes, ...aiRoutes, ...liveRoutes];
    return allRoutes.map(r => ({ method: r.method, path: r.path, requiresAuth: r.requiresAuth ?? true }));
  }

  getHealthStatus() {
    return { status: 'healthy', service: 'quantmax', uptime: process.uptime(), version: '1.0.0', routes: this.getRoutes().length, features: ['short-videos', 'video-chat', 'matching', 'safety', 'live-events', 'ai-moderation'] };
  }
  ...
  start(): void {
    ...
    server.listen(this.config.port, this.config.host, () => {
      console.log(`[QuantMax] Server running at http://${this.config.host}:${this.config.port}`);
      console.log(`[QuantMax] Routes registered: ${this.getRoutes().length}`);
    });
  }
}

export default QuantMaxServer;
