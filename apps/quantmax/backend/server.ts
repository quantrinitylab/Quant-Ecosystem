/**
 * Production entrypoint for the QuantMax Fastify backend.
 *
 ...
 * handlers all proxy to `QUANTMAX_BACKEND_URL`, with nothing listening on the
...
async function main(): Promise<void> {
  if (process.env['NODE_ENV'] === 'test') {
    throw new Error('FATAL: QuantMax server cannot boot with NODE_ENV=test in standalone process');
  }
  ...
  try {
    ...
    app.log.info({ port: config.port, host: config.host }, 'QuantMax backend listening');
  } catch (err) {
    app.log.error({ err }, 'Failed to start QuantMax backend');
    process.exit(1);
  }
}
