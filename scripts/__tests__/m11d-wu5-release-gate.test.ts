...
describe('M11d WU5 memory release gate', () => {
  ...
  it('blocks cloud credentials and image push until memory preflight succeeds', () => {
    ...
    const deploy = workflow.indexOf('  build-and-deploy:');
    const needs = workflow.indexOf('needs: [validate-release, memory-release-preflight]', deploy);
    const aws = workflow.indexOf('Configure AWS credentials with OIDC', deploy);
    const push = workflow.indexOf('Build and push SHA-tagged image', deploy);
