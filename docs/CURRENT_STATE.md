...
| QuantAI canary  | [WU2 fail-closed evidence](./baselines/m11d-fail-closed-contract-2026-07-22.md) verifies structured mode errors, durable dependency tags, blocked `new` authority, and 15/15 focused tests.  | Explicit non-legacy modes no longer start with process-local substitutes; shadow remains unavailable until WU3 wires durable vector/report dependencies. |
...
Execute work unit 3 in the [Execution Queue](./EXECUTION_QUEUE.md): persist tenant-scoped shadow reports across restart, wire the real vector/report composition, and prove isolation without enabling `new` authority.
