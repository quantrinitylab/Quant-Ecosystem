...
`.github/workflows/deploy.yml` runs `memory-release-preflight` without cloud credentials. Every production deployment job depends on both immutable-release validation and this preflight; a failure prevents AWS authentication and image push/rollout. The existing protected GitHub `production` Environment remains mandatory.
